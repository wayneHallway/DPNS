...
class LightGCN1(nn.Module):
        ...
        if self.ns == 'rns':  # n_negs = 1
            ...
            for k in range(self.K):
                neg_gcn_embs.append(self.dynamic_negative_sampling(user_gcn_emb, item_gcn_emb,
                                                                   user,
                                                                   neg_item[:, k * self.n_negs: (k + 1) * self.n_negs],cur_epoch))
    ...
    def dynamic_negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates,cur_epoch):
        ...
        """Add random noise to s_e"""
        random_noise = torch.rand_like(s_e).cuda()
        normalized_noise = F.normalize(random_noise, dim=-1)  # Normalize random noise
        ...
        scores2 = (s_e1.unsqueeze(dim=1) * n_e).sum(dim=-1)  # [batch_size, n_negs]
        scores3 = (s_e2.unsqueeze(dim=1) * n_e).sum(dim=-1)  # [batch_size, n_negs]

        scores_avg = ((1 - min(0.6, cur_epoch / 2*self.warmup)) *scores1 + min(0.6, cur_epoch / 2*self.warmup)*(scores2 + scores3)) / 3.0  # Average scores
        indices = torch.max(scores1, dim=1)[1].detach()  # [batch_size]
